...
def game_window_init():
   ...
   # storing images (Monopoly board and sample money)
    ...
    # if player has passed go, amount and networth get incremented by 200
    ...
    # is_member: listof [string, int], string -> Bool
    ...
    class Button:
        def __init__(self, surface, rect_colour, x, y, width, height, text_to_display, font, text_colour, border_colour, border_rect):
            ...
            self.font = font # button font (the sysfont stuff)
   ...
   # initialising variables for player coordinates, amount and networth
    ...
    # blitting text for the amounts and networths outside the board
    ...
    properties_unowned = [["REV", 1000], ["MKV", 1200], ["M3", 2000], ["MC", 2200], ["Mudie's", 3000],
                          ["V1", 3200], ["UWP", 4000], ["CMH", 4200], ["DC Bytes", 5000], ["DC", 5200],
                          ["PAC", 6000], ["SLC", 6200], ["STC", 7000], ["DP", 7200], ["Tim Hortons", 8000],
                          ["QNC", 8200]]
    p1_owned = []
    p2_owned = []


    # initializing variables to control the flow of the game (whose turn it is and endgame status)
    ...
    def player1_turn(property_name, property_price, indicator_rect):
        ...
        # if the property is unknowned and the player wants to buy the property
        if is_member(properties_unowned, property_name) and event.type == pygame.MOUSEBUTTONDOWN and buy_button.return_virtual_rect().collidepoint(event.pos):
            ...
            counter = 2
                ...
                event.pos):
            # drawing rect over old p1 amount and blitting the new p1 amounts and networths
            ...
            # drawing rect over old p2 amonut and blitting the new p1 amounts and networths
    ...
    while is_running:
        for event in pygame.event.get():
            ...
            if endgame == 0:
                # if player 1 rolls
                if event.type == pygame.MOUSEBUTTONDOWN and roll_dice_button.return_virtual_rect().collidepoint(event.pos):
                ...
                # if player 2 rolls
                if event.type == pygame.MOUSEBUTTONDOWN and roll_dice_button_2.return_virtual_rect().collidepoint(event.pos):
                    number_rolled = (random.randrange(1, 7))
                ...
                # if counter == 1:
                    ...
                    # if p1_x == 416 and p1_y == 630:
                        # CALL THE FUNCTION WITH PARAMETERS FOR MKV
                        # counter = 2
                    # elif p1_x == 232 and p1_y == 630:
                        # CALL THE FUNCTION WITH PARAMETERS FOR M3
                        # counter = 2
                if counter == 1:
                    #if player1 lands on MKV
                    if p1_x == 416 and p1_y == 630:
                        if is_member(properties_unowned, 'MKV') and event.type == pygame.MOUSEBUTTONDOWN and buy_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, "red", (393, 669, 92, 10)) #indicator
                            p1_owned.append(["MKV", 1200])
                            pygame.draw.rect(screen, "black", (910, 100, 90, 50))
                            blit_text(str(p1_amount - 1200), amount_p1_font, "red", 910, 100)
                            p1_amount -= 1200
                            pygame.draw.rect(screen, 'black', (1140, 100, 90, 50))
                            blit_text(str(p1_networth), p1_font, 'red', 1140, 100)
                            blit_text(str(p2_networth), p2_font, 'blue', 1140, 600)
                            properties_unowned.remove(["MKV", 1200])
                            counter = 2
                        elif is_member(properties_unowned, "MKV") and event.type == pygame.MOUSEBUTTONDOWN and dont_buy_button.return_virtual_rect().collidepoint(event.pos):
                            counter = 2
                        elif is_member(p1_owned, "MKV"):
                            counter = 2
                        elif is_member(p2_owned, "MKV") and event.type == pygame.MOUSEBUTTONDOWN and pay_rent_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, (0, 0, 0), (910, 100, 90, 50))
                            blit_text(str(p1_amount - 600), amount_p1_font, "red", 910, 100)
                            pygame.draw.rect(screen, (0, 0, 0), (910, 600, 90, 50))
                            blit_text(str(p2_amount + 600), amount_p1_font, "blue", 910, 600)
                            p1_amount -= 600
                            p1_networth -= 600
                            p2_amount += 600
                            p2_networth += 600
                    ...
                    if p1_x == 232 and p1_y == 630:
                        if is_member(properties_unowned, 'M3') and event.type == pygame.MOUSEBUTTONDOWN and buy_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, "red", (209, 669, 92, 10))
                            p1_owned.append(["M3", 2000])
                            pygame.draw.rect(screen, (0, 0, 0), (910, 100, 90, 50))
                            blit_text(str(p1_amount - 2000), amount_p1_font, "red", 910, 100)
                            p1_amount -= 2000
                            pygame.draw.rect(screen, 'black', (1140, 100, 90, 50))
                            blit_text(str(p1_networth), p1_font, 'red', 1140, 100)
                            blit_text(str(p2_networth), p2_font, 'blue', 1140, 600)
                            properties_unowned.remove(["M3", 2000])
                            counter = 2
                        elif is_member(properties_unowned, "M3") and event.type == pygame.MOUSEBUTTONDOWN and dont_buy_button.return_virtual_rect().collidepoint(event.pos):
                            counter = 2
                        elif is_member(p1_owned, "M3"):
                            counter = 2
                        elif is_member(p2_owned, "M3") and event.type == pygame.MOUSEBUTTONDOWN and pay_rent_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, (0, 0, 0), (910, 100, 90, 50))
                            blit_text(str(p1_amount - 800), amount_p1_font, "red", 910, 100)
                            pygame.draw.rect(screen, (0, 0, 0), (910, 600, 90, 50))
                            blit_text(str(p2_amount + 800), amount_p1_font, "blue", 910, 600)
                            p1_amount -= 800
                            p1_networth -= 800
                            p2_amount += 800
                            p2_networth += 800
                ...
                # if it is player 2s turn
                    ...
                    # landing on MKV
                    if p2_x == 456 and p2_y == 630:  # and counter % 2 == 1:  # we are in MKV
                        if is_member(properties_unowned, 'MKV') and event.type == pygame.MOUSEBUTTONDOWN and buy_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, "blue", (393, 669, 92, 10))
                            p2_owned.append(["MKV", 1200])
                            pygame.draw.rect(screen, (0, 0, 0), (910, 600, 90, 50)) #rectangle to put over the old amount
                            blit_text(str(p2_amount - 1200), amount_p2_font, "blue", 910, 600) #blitting the new amount
                            p2_amount -= 1200
                            pygame.draw.rect(screen, 'black', (1140, 600, 90, 50))
                            blit_text(str(p1_networth), p1_font, 'red', 1140, 100)
                            blit_text(str(p2_networth), p2_font, 'blue', 1140, 600)
                            ...
                            counter = 1
                        elif is_member(p2_owned, "MKV"):
                            counter = 1
                        elif is_member(p1_owned, "MKV") and event.type == pygame.MOUSEBUTTONDOWN and pay_rent_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, (0, 0, 0), (910, 600, 90, 50))
                            blit_text(str(p2_amount - 600), amount_p2_font, "blue", 910, 600)
                            pygame.draw.rect(screen, (0, 0, 0), (910, 100, 90, 50))
                            blit_text(str(p1_amount + 600), amount_p1_font, "red", 910, 100)
                            p2_amount = p2_amount - 600
                            p2_networth -= 600
                            p1_amount = p1_amount + 600
                            p1_networth += 600
                            pygame.draw.rect(screen, 'black', (1140, 100, 90, 50))
                            pygame.draw.rect(screen, 'black', (1140, 600, 90, 50))
                            blit_text(str(p1_networth), p1_font, 'red', 1140, 100)
                    ...
                    # landing on M3
                    if p2_x == 272 and p2_y == 630:  # and counter % 2 == 1:  # we are in M3
                        if is_member(properties_unowned, 'M3') and event.type == pygame.MOUSEBUTTONDOWN and buy_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, "blue", (209, 669, 92, 10))
                            p2_owned.append(["M3", 2000])
                            pygame.draw.rect(screen, (0, 0, 0), (910, 600, 90, 50))
                            blit_text(str(p2_amount - 2000), amount_p2_font, "blue", 910, 600)
                            p2_amount -= 2000
                            pygame.draw.rect(screen, 'black', (1140, 600, 90, 50))
                            blit_text(str(p1_networth), p1_font, 'red', 1140, 100)
                            blit_text(str(p2_networth), p2_font, 'blue', 1140, 600)
                            ...
                            counter = 1
                        elif is_member(p2_owned, "M3"):
                            counter = 1
                        elif is_member(p1_owned, "M3") and event.type == pygame.MOUSEBUTTONDOWN and pay_rent_button.return_virtual_rect().collidepoint(event.pos):
                            pygame.draw.rect(screen, (0, 0, 0), (910, 600, 90, 50))
                            blit_text(str(p2_amount - 800), amount_p2_font, "blue", 910, 600)
                            pygame.draw.rect(screen, (0, 0, 0), (910, 100, 90, 50))
                            blit_text(str(p1_amount + 800), amount_p1_font, "red", 910, 100)
                            p2_amount = p2_amount - 800
                            p2_networth -= 800
                            p1_amount = p1_amount + 800
                            p1_networth += 800
                            pygame.draw.rect(screen, 'black', (1140, 100, 90, 50))
                            pygame.draw.rect(screen, 'black', (1140, 600, 90, 50))
                            blit_text(str(p1_networth), p1_font, 'red', 1140, 100)
